...
class BundesligaUltimatePredictor:
    ...
    def _combine_predictions(self, all_predictions):
        ...
        if 'error' in base or 'error' in poisson:
            return {
                'prediction': 'Unknown',
                'confidence': 0.0,
                'probabilities': {'Home Win': 0.33, 'Draw': 0.33, 'Away Win': 0.33},
                'predicted_score': 'N/A',
                'score_probabilities': {}
            }
        ...
        base_probs = base.get('probabilities', {'Home Win': 0.33, 'Draw': 0.33, 'Away Win': 0.33})
        poisson_probs = {
    ...
    def predict_upcoming_matches(self):
        """
        Gelecek hafta maçlarını tahmin et - OpenLigaDB API'den gerçek maçları çek
        """
        import requests
        
        try:
            # OpenLigaDB API'den güncel maçları çek
            response = requests.get('https://api.openligadb.de/getmatchdata/bl1', timeout=10)
            if response.status_code != 200:
                return []
            
            api_matches = response.json()
            
            # Tamamlanmamış maçları filtrele
            upcoming_matches = [m for m in api_matches if not m.get('matchIsFinished', True)]
            
            if not upcoming_matches:
                return []
            
            # İlk maçın hafta numarasını al
            current_matchday = upcoming_matches[0].get('group', {}).get('groupOrderID', 28)
            
            # Sadece o haftanın maçlarını al
            matchday_matches = [m for m in upcoming_matches 
                               if m.get('group', {}).get('groupOrderID') == current_matchday]
        ...
        for match in matchday_matches[:9]:  # İlk 9 maç
            home_name = match.get('team1', {}).get('teamName', '')
            away_name = match.get('team2', {}).get('teamName', '')
            match_dt = match.get('matchDateTime', '')
            match_id = match.get('matchID', 0)
            
            # Takım ID'lerini veritabanından bul
            home_id = conn.execute('SELECT id FROM teams WHERE name = ?', (home_name,)).fetchone()
            away_id = conn.execute('SELECT id FROM teams WHERE name = ?', (away_name,)).fetchone()
